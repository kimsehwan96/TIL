...
전통적인 RDBMS의 경우는 스케일 업 전략을 취하고, NoSQL 은 여러 샤드로 스케일 아웃 하는 전략을 취한다.

이는 NoSQL 의 탄생 목적 자체가 대규모 트래픽/데이터를 처리하기 위해 도입되고, 또 스케일아웃을 고려하여 설계되었기 때문이다.
...
Parition key + Sort key 를 Primary Key 라고 부를 수 있는데. 오직 Primary Key 로만 데이터를 쿼리 가능하고, 다른 어트리뷰트에 대해서는 Scan API 등을 통해서 데이터 조회가 가능함 (프로덕션 환경에서는 사용하지 않을 Scan)

## 기억해야 할 제약 조건

DynamoDB 에서 테이블을 생성하면 내부적으로 파티션을 나눠서 리소스를 제공하고, 이 때 이러한 사항을 고객에게 노출하지 않고 DynamoDB 내부적으로 처리하는데, 이 때 하나의 파티션 별로 초당 1K WCU (쓰기 작업), 3K RCU (읽기 작업), 10GB 의 데이터를 저장
...
- 테이블에 원하는 양의 처리량 설정
- RCU: 4 KB/s(프로비전드) or request(온디맨드)
- WCU: 1 KB/s(프로비전드) or request(온디맨드)
- RCU/WCU 는 독립적으로 동작
- Eventually Consistent 읽기는 Strongly Consistent 읽기에 비해 RCU를 절반만 사용: 8 KB/s(프로비전드) or request(온디맨드)

사이즈

- 테이블에 아이템을 원하는 수만큼 추가 가능
- 아이템의 최대 크기는 400KB
- 하지만 400KB 를 모두 사용하는 것은 비권장

DynamoDB 에서 가져갈 전략은, 작은 데이터를 매우 많이 저장하는것이 올바른 접근 법

스케일링은 파티셔닝을 통해 이루어지고. 각 파티션세어 1K WCU / 3K RCU 중 둘중 하나가 초과되면 파티션이 늘어남, 데이터 용량이 10GB를 넘을 때에도 파티션이 늘어남

## 키 디자인

DynamoDB 의 각 파티션별 읽기/쓰기 그리고 처리가능한 데이터의 양에 대한 제약 조건은 변하지 않고, 파티션의 수가 늘어 날 수 는 있음. 따라서 키 디지인시 가장 신경 써야 하는 부분은. 최대한 모든 파티션이 골고루 사용되도록 하는 것. 이 때 Partition Key 의 값을 `OrderId: 1`, `OrderId: 2`, `OrderId: 3`, 등으로 1,2,3 과 값은 값을 지정했더라도, DynamoDB 는 내부적으로 파티션 키 `값` 에 대해서 해쉬 함수를 통해 해싱한뒤 저장하기 때문에, 굳이 UUID나, 아니면 어려운 값을 쓰지 않아도 DynamoDB 가 내부적으로 아이템을 파티션별로 분산 저장되도록 함.

### 키 디자인 풀 사이클

1. 비즈니스 유즈 케이스 이해하기
2. ER 다이어그램 그리기
3. 모든 데이터 액세스 패턴 정리하기
4. 키 디자인 시작하기

ER 다이어그램은 각 엔티티의 속성까지 그릴 필요는 없고. 엔티티간의 관계만 정의 가능하도록 그린다. (1:n , n:m, 1:1 등의 관계만 확인하기 위해)

### 비정규화

DynamoDB가 잘 하는 것은 무한하게 많은 아이템중 하나 혹은 몇개를 일정한 시간에 찾아내는 것

RDMBS 는 데이터 액세스 패턴을 고민하기 보다는 데이터 모델 정규화를 위해 노력함. Ad-hoc 쿼리도 가능

DynamoDB 는 모든 데이터 액세스 패턴을 알고 시작해야 함. Pre-computation 필요

### PK+SK 복합키

SK(Sort Key) : 알파벳, 숫자, 타임스탬프,ULID/KSUID 등을 많이 사용하며, 오름차순, 내림차순 조회가 가능함
IoT 로그, 소셜 네트워크에 포스트 리스트, 주문 상세 정보 혹은 이력 등에 사용함

### 안티패턴

- PK를 UserID로 고정하고 시작하는 습간
    - 일반 사용자와 VIP를 같은 키 디자인으로 해결하려는 습관
    - 대량트래픽을 유발하는 헤비유저를 고민해야 함
- 엔티티 별로 테이블을 만드려는 습관
- GSI를 많이 사용하려는 습관
...
데이터는 항상 3개의 가용영역 (AZ)에 복제되어 저장되고, 서비스 또한 3개의 가용 영역에서 실행 됨.

## DynamoDB 에서 데이터를 읽는 방법
...
- 일치하는 아이템 반환 가능(여러 개 가능)
- Key 조건과 일치하는 아이템의 크기에 따라 RCU를 소비하여 단일 결과를 반환

SCAN
- 선택적으로 키가 아닌 어트리뷰트에 대한 필터 조건 지정
- 필터 표현식과 일치하는 테이블의 모든 아이템을 반환
- 테이블의 모든 아이템을 읽기 위해 RCU 소비. 운영 환경에서 사용을 하는것은 주의해야 함

### API 제약 조건

### Query / Scan

단일 호출로 최대 1MB를 반환하며, 응답 메시지가 1MB 이상 시 LastEvaulatedKey 를 이용해 페이지네이션 가능

### BatchGetItem

단일 호출로 최대 100개의 아이템, 혹은 최대 16MB 반환

### BatchWriteItem

단일 호출로 최대 25 PutItem 혹은 DeleteItem 수행하며 최대 16MB 처리

## 싱글테이블 설계 에시

ref : https://github.com/aws-samples/amazon-dynamodb-design-patterns/tree/master/examples/an-online-shop